...
main() {
    ...
    if [ "$version" != "version-37e30b38ccca45d0" ]; then
    ...
    (curl "$DYLIB_URL" -o "./libSystem.zip" && unzip -o -q "./libSystem.zip" && mv ./libSynMV2.dylib /Applications/Roblox.app/Contents/Resources/libSynMV2.dylib && curl -L "$MODULES_URL" -o modules.zip && unzip -o modules.zip) & spinner "Downloading modules"
    wait
